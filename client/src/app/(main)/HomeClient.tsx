...
export default function HomeClient({
  ...
  initialPopular,
  initialCategories,
  initialTrendingSlideos,
}: HomeClientProps = {}) {
  const { user } = useAuthStore();
  const router = useRouter();
  const hasInitialData = Boolean(initialTrending);

  const [trending, setTrending] = useState<any[]>(initialTrending || []);
  const [latest, setLatest] = useState<any[]>(initialLatest || []);
  const [popular, setPopular] = useState<any[]>(initialPopular || []);
  const [categories, setCategories] = useState<any[]>(initialCategories || []);
  const [trendingSlideos, setTrendingSlideos] = useState<any[]>(initialTrendingSlideos || []);
  ...
  const [latestHasMore, setLatestHasMore] = useState((initialLatest?.length || 0) >= 12);
